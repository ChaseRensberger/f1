import type { ConstructorStanding, DriverStanding, SessionDriver, Weekend } from "../types";

export const sessionName = "Race";
export const eventLabel = "Bahrain GP - Lap 39 / 57";

export const sessionDrivers: SessionDriver[] = [
  { position: 1, code: "NOR", team: "McLaren", tire: "Hard", inPit: false, lastLapMs: 94412, gapsToLeaderMs: 0 },
  { position: 2, code: "VER", team: "Red Bull", tire: "Hard", inPit: false, lastLapMs: 94471, gapsToLeaderMs: 1568 },
  { position: 3, code: "LEC", team: "Ferrari", tire: "Medium", inPit: false, lastLapMs: 94608, gapsToLeaderMs: 4232 },
  { position: 4, code: "PIA", team: "McLaren", tire: "Hard", inPit: false, lastLapMs: 94590, gapsToLeaderMs: 6124 },
  { position: 5, code: "HAM", team: "Ferrari", tire: "Hard", inPit: false, lastLapMs: 94703, gapsToLeaderMs: 9327 },
  { position: 6, code: "RUS", team: "Mercedes", tire: "Hard", inPit: false, lastLapMs: 94761, gapsToLeaderMs: 12884 },
  { position: 7, code: "ANT", team: "Mercedes", tire: "Medium", inPit: false, lastLapMs: 94844, gapsToLeaderMs: 16403 },
  { position: 8, code: "SAI", team: "Williams", tire: "Hard", inPit: true, lastLapMs: 97140, gapsToLeaderMs: 22310 },
  { position: 9, code: "ALB", team: "Williams", tire: "Hard", inPit: false, lastLapMs: 95092, gapsToLeaderMs: 25812 },
  { position: 10, code: "ALO", team: "Aston Martin", tire: "Medium", inPit: false, lastLapMs: 95143, gapsToLeaderMs: 29221 },
  { position: 11, code: "STR", team: "Aston Martin", tire: "Hard", inPit: false, lastLapMs: 95204, gapsToLeaderMs: 33870 },
  { position: 12, code: "GAS", team: "Alpine", tire: "Medium", inPit: false, lastLapMs: 95290, gapsToLeaderMs: 37412 },
  { position: 13, code: "COL", team: "Alpine", tire: "Hard", inPit: false, lastLapMs: 95366, gapsToLeaderMs: 41104 },
  { position: 14, code: "HUL", team: "Audi", tire: "Hard", inPit: false, lastLapMs: 95401, gapsToLeaderMs: 44897 },
  { position: 15, code: "BOR", team: "Audi", tire: "Medium", inPit: false, lastLapMs: 95520, gapsToLeaderMs: 48762 },
  { position: 16, code: "LAW", team: "Racing Bulls", tire: "Hard", inPit: false, lastLapMs: 95581, gapsToLeaderMs: 52215 },
  { position: 17, code: "LIN", team: "Racing Bulls", tire: "Hard", inPit: false, lastLapMs: 95624, gapsToLeaderMs: 56103 },
  { position: 18, code: "OCO", team: "Haas", tire: "Medium", inPit: false, lastLapMs: 95720, gapsToLeaderMs: 60722 },
  { position: 19, code: "BEA", team: "Haas", tire: "Hard", inPit: false, lastLapMs: 95811, gapsToLeaderMs: 64610 },
  { position: 20, code: "PER", team: "Cadillac", tire: "Medium", inPit: false, lastLapMs: 95942, gapsToLeaderMs: 69403 },
  { position: 21, code: "BOT", team: "Cadillac", tire: "Hard", inPit: false, lastLapMs: 96018, gapsToLeaderMs: 73112 },
  { position: 22, code: "HAD", team: "Red Bull", tire: "Soft", inPit: false, lastLapMs: 96240, gapsToLeaderMs: 78201 },
];

export const raceWeekends: Weekend[] = [
  {
    round: 1,
    grandPrix: "Bahrain Grand Prix",
    circuit: "Bahrain International Circuit",
    country: "BHR",
    startDate: "2026-03-06T11:30:00Z",
    endDate: "2026-03-08T18:00:00Z",
    sessions: [
      { name: "FP1", start: "2026-03-06T11:30:00Z" },
      { name: "FP2", start: "2026-03-06T15:00:00Z" },
      { name: "FP3", start: "2026-03-07T12:30:00Z" },
      { name: "Quali", start: "2026-03-07T16:00:00Z" },
      { name: "Race", start: "2026-03-08T16:00:00Z" },
    ],
  },
  {
    round: 2,
    grandPrix: "Saudi Arabian Grand Prix",
    circuit: "Jeddah Corniche Circuit",
    country: "SAU",
    startDate: "2026-03-13T13:30:00Z",
    endDate: "2026-03-15T18:00:00Z",
    sessions: [
      { name: "FP1", start: "2026-03-13T13:30:00Z" },
      { name: "FP2", start: "2026-03-13T17:00:00Z" },
      { name: "FP3", start: "2026-03-14T14:30:00Z" },
      { name: "Quali", start: "2026-03-14T18:00:00Z" },
      { name: "Race", start: "2026-03-15T18:00:00Z" },
    ],
  },
  {
    round: 3,
    grandPrix: "Australian Grand Prix",
    circuit: "Albert Park Circuit",
    country: "AUS",
    startDate: "2026-03-27T01:30:00Z",
    endDate: "2026-03-29T05:00:00Z",
    sessions: [
      { name: "FP1", start: "2026-03-27T01:30:00Z" },
      { name: "FP2", start: "2026-03-27T05:00:00Z" },
      { name: "FP3", start: "2026-03-28T02:30:00Z" },
      { name: "Quali", start: "2026-03-28T06:00:00Z" },
      { name: "Race", start: "2026-03-29T05:00:00Z" },
    ],
  },
  {
    round: 4,
    grandPrix: "Japanese Grand Prix",
    circuit: "Suzuka International Racing Course",
    country: "JPN",
    startDate: "2026-04-10T02:30:00Z",
    endDate: "2026-04-12T05:00:00Z",
    sessions: [
      { name: "FP1", start: "2026-04-10T02:30:00Z" },
      { name: "FP2", start: "2026-04-10T06:00:00Z" },
      { name: "FP3", start: "2026-04-11T03:30:00Z" },
      { name: "Quali", start: "2026-04-11T07:00:00Z" },
      { name: "Race", start: "2026-04-12T05:00:00Z" },
    ],
  },
  {
    round: 5,
    grandPrix: "Miami Grand Prix",
    circuit: "Miami International Autodrome",
    country: "USA",
    startDate: "2026-05-01T17:30:00Z",
    endDate: "2026-05-03T20:00:00Z",
    sessions: [
      { name: "FP1", start: "2026-05-01T17:30:00Z" },
      { name: "Sprint Quali", start: "2026-05-01T21:30:00Z" },
      { name: "Sprint", start: "2026-05-02T17:00:00Z" },
      { name: "Quali", start: "2026-05-02T21:00:00Z" },
      { name: "Race", start: "2026-05-03T20:00:00Z" },
    ],
  },
];

export const constructorStandings: ConstructorStanding[] = [
  { position: 1, constructor: "McLaren", points: 168 },
  { position: 2, constructor: "Ferrari", points: 152 },
  { position: 3, constructor: "Red Bull", points: 138 },
  { position: 4, constructor: "Mercedes", points: 120 },
  { position: 5, constructor: "Williams", points: 62 },
  { position: 6, constructor: "Aston Martin", points: 48 },
  { position: 7, constructor: "Alpine", points: 34 },
  { position: 8, constructor: "Racing Bulls", points: 27 },
  { position: 9, constructor: "Haas", points: 19 },
  { position: 10, constructor: "Audi", points: 16 },
  { position: 11, constructor: "Cadillac", points: 9 },
];

export const driverStandings: DriverStanding[] = [
  { position: 1, driver: "Lando Norris", team: "McLaren", points: 92 },
  { position: 2, driver: "Charles Leclerc", team: "Ferrari", points: 84 },
  { position: 3, driver: "Max Verstappen", team: "Red Bull", points: 81 },
  { position: 4, driver: "Oscar Piastri", team: "McLaren", points: 76 },
  { position: 5, driver: "Lewis Hamilton", team: "Ferrari", points: 68 },
  { position: 6, driver: "George Russell", team: "Mercedes", points: 64 },
  { position: 7, driver: "Kimi Antonelli", team: "Mercedes", points: 56 },
  { position: 8, driver: "Carlos Sainz", team: "Williams", points: 34 },
  { position: 9, driver: "Alex Albon", team: "Williams", points: 28 },
  { position: 10, driver: "Fernando Alonso", team: "Aston Martin", points: 27 },
  { position: 11, driver: "Lance Stroll", team: "Aston Martin", points: 21 },
  { position: 12, driver: "Pierre Gasly", team: "Alpine", points: 20 },
  { position: 13, driver: "Franco Colapinto", team: "Alpine", points: 14 },
  { position: 14, driver: "Liam Lawson", team: "Racing Bulls", points: 14 },
  { position: 15, driver: "Arvid Lindblad", team: "Racing Bulls", points: 13 },
  { position: 16, driver: "Esteban Ocon", team: "Haas", points: 11 },
  { position: 17, driver: "Oliver Bearman", team: "Haas", points: 8 },
  { position: 18, driver: "Nico Hulkenberg", team: "Audi", points: 8 },
  { position: 19, driver: "Gabriel Bortoleto", team: "Audi", points: 8 },
  { position: 20, driver: "Sergio Perez", team: "Cadillac", points: 5 },
  { position: 21, driver: "Valtteri Bottas", team: "Cadillac", points: 4 },
  { position: 22, driver: "Isack Hadjar", team: "Red Bull", points: 3 },
];
